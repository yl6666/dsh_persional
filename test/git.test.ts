...
import { GitClient, NodeCommandRunner } from '../src/exec/git.ts'
...
describe('GitClient parsing and arguments (fake runner)', () => {
  ...
  it('parses a plain branch header and reports clean', async () => {
    const runner = fakeRunner({
      'status --porcelain=v1 --branch': { stdout: '## feature/x', stderr: '', exitCode: 0 },
    })
  ...
  it('commitAll stages everything, commits with identity overrides, returns the hash', async () => {
    const runner = fakeRunner({
      ...
      '-c user.name=board -c user.email=b@x commit -m msg': { stdout: '[main abc1234] msg', stderr: '', exitCode: 0 },
...
describe('GitClient against a real git checkout', () => {
  ...
  it('init -> status -> commitAll -> clean status round trip', async () => {
