...
function parseSpec(value: unknown): RequirementSpec {
...
function parsePlan(value: unknown): RepoModificationPlan {
...
function parseQuestions(value: unknown): ClarificationQuestion[] {
  if (!Array.isArray(value)) fail('questions must be an array')
...
function scanTool(ctx: Context): RawToolDefinition {
  return {
    ...
    description:
      '扫描一组本地代码仓库，构建/更新多仓关系图谱（构建依赖 + 契约提供方/消费方自动抽取）。' +
    ...
    async execute(args) {
      ...
      if (parsedRepos.length === 0) fail('repos must not be empty')
...
function graphTool(ctx: Context): RawToolDefinition {
  return {
    name: 'repo_board_graph',
    description:
      '查询当前多仓关系图谱：document（全图）、impact（某仓库的反向影响面/爆炸半径）、' +
    ...
    async execute(args) {
      ...
      const board = (ctx as { repoBoard: RepoBoardService }).repoBoard
      if (action === 'document') return summarizeGraph(board.document)
      const repo = asString(input['repo'], 'repo')
      if (action === 'impact') return { repo, impact: board.impact(repo, options) }
      if (action === 'dependencies') return { repo, dependencies: board.dependencies(repo, options) }
      if (action === 'topology') {
        return {
...
function dispatchTool(ctx: Context): RawToolDefinition {
  return {
    name: 'repo_board_dispatch',
    description:
      '登记一条多仓需求（一次下发的入口）。返回需求 id 与当前状态。' +
    ...
    async execute(args) {
      ...
      if (input['candidateRepos'] !== undefined) hint['candidateRepos'] = asStringArray(input['candidateRepos'], 'candidateRepos')
      if (input['allowBreaking'] !== undefined) hint['allowBreaking'] = asBoolean(input['allowBreaking'], 'allowBreaking')
      if (input['priority'] !== undefined) {
        const priority = asString(input['priority'], 'priority')
        if (priority !== 'low' && priority !== 'normal' && priority !== 'high') fail('priority must be low, normal, or high')
      ...
      return {
        ...
        next:
          '信息充分则调用 repo_board_spec 提交规格；信息不足则先向用户追问，' +
...
function clarifyTool(ctx: Context): RawToolDefinition {
  return {
    name: 'repo_board_clarify',
    description:
      '记录一次需求澄清批次（[0] 追问闸门）：登记你向用户提出的问题与收到的回答。' +
    ...
    async execute(args) {
      ...
      const session = updated.toDocument().clarification as { questions: unknown[]; answers?: Record<string, string> }
      ...
      return {
        ...
        next:
          answered.length === blocking.length
...
function specTool(ctx: Context): RawToolDefinition {
  return {
    name: 'repo_board_spec',
    description:
      '提交完整需求规格（[0]+[1]），自动完成影响面分析（[2]）并生成各仓方案骨架（[3] 草稿）。' +
    ...
    async execute(args) {
      ...
      return {
        ...
        next:
          '填写每仓方案的 summary/changes/writeScopes/contractImpact.breaking（prerequisites 已由图谱推导，' +
...
function plansTool(ctx: Context): RawToolDefinition {
  return {
    name: 'repo_board_plans',
    description:
      '挂载填好的各仓修改方案（[3]）并生成人审发现（[4]）：写范围冲突、声明为破坏性的契约变更、关键契约边。' +
    ...
    async execute(args) {
      ...
      return {
        ...
        next:
          findings.conflicts.length > 0 || findings.breakingChanges.length > 0
...
function executeTool(ctx: Context): RawToolDefinition {
  return {
