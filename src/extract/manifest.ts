...
function parseJson<T>(repo: string, file: string, content: string): T {
  try {
...
function parseNpm(repo: string, content: string, result: { provides: PackageName[]; dependsOn: DependencyDeclaration[] }): void {
  const json = parseJson<NpmManifest>(repo, 'package.json', content)
...
function parseGoMod(content: string, result: { provides: PackageName[]; dependsOn: DependencyDeclaration[] }): void {
  const module = /^module\s+(\S+)/m.exec(content)?.[1]
  if (module !== undefined) result.provides.push({ ecosystem: 'go', name: module })
  const requireLine = (name: string, version: string, rest: string): void => {
    if (rest.includes('indirect')) return
  ...
  for (const block of content.matchAll(/^require\s*\(([\s\S]*?)\)/gm)) {
    for (const line of block[1]!.split('\n')) {
      ...
      if (m !== null) requireLine(m[1]!, m[2]!, m[3] ?? '')
    }
  }
}

function parseCargoToml(content: string, result: { provides: PackageName[]; dependsOn: DependencyDeclaration[] }): void {
  let section = ''
  for (const rawLine of content.split('\n')) {
    ...
    if (section === 'package') {
      ...
      if (m !== null) result.provides.push({ ecosystem: 'cargo', name: m[1]! })
...
function parsePomXml(content: string, result: { provides: PackageName[]; dependsOn: DependencyDeclaration[] }): void {
...
function parsePep508(spec: string): DependencyDeclaration {
...
function parsePyproject(content: string, result: { provides: PackageName[]; dependsOn: DependencyDeclaration[] }): void {
  let section = ''
  ...
  for (const rawLine of content.split('\n')) {
    ...
    if (inDepsArray) {
      ...
      if (item !== undefined) result.dependsOn.push(parsePep508(item))
      continue
    }
    ...
    if (/^dependencies\s*=\s*\[/.test(line)) {
      ...
      if (inline !== undefined && inline.trim() !== '') {
        for (const item of inline.matchAll(/["']([^"']+)["']/g)) result.dependsOn.push(parsePep508(item[1]!))
      } else {
        inDepsArray = true
      }
    }
...
function parseRequirements(content: string, result: { dependsOn: DependencyDeclaration[] }): void {
  for (const rawLine of content.split('\n')) {
...
function parseGemfile(content: string, result: { dependsOn: DependencyDeclaration[] }): void {
...
function parseGemspec(content: string): string | undefined {
...
function parseComposer(repo: string, content: string, result: { provides: PackageName[]; dependsOn: DependencyDeclaration[] }): void {
  const json = parseJson<ComposerManifest>(repo, 'composer.json', content)
  if (typeof json.name === 'string' && json.name !== '') {
 ...
 */
export function parseManifests(repo: string, files: Readonly<Record<string, string>>): ManifestScanResult {
  const provides: PackageName[] = []
  const dependsOn: DependencyDeclaration[] = []
  ...
  if (requirementsContent !== undefined) {
    if (!buildSystems.includes('pypi')) buildSystems.push('pypi')
  ...
  for (const [path, content] of Object.entries(files)) {
    if (!path.endsWith('.gemspec')) continue
 ...
 */
export function buildEdgesFromManifests(scans: readonly ManifestScanResult[]): AutoEdgeInput[] {
  const providers = new Map<string, string>()
  for (const scan of scans) {
    for (const pkg of scan.provides) providers.set(pkg.ecosystem + '::' + pkg.name, scan.repo)
  }
  const edges = new Map<string, AutoEdgeInput>()
  for (const scan of scans) {
    for (const dep of scan.dependsOn) {
      const target = providers.get(dep.ecosystem + '::' + dep.name)
      if (target === undefined || target === scan.repo) continue
 ...
 */
export function contentFingerprint(files: Readonly<Record<string, string>>): string {
