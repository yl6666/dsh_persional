...
function detachContractRef(ref: ContractRef | undefined): ContractRef | undefined {
  return ref === undefined ? undefined : { ...ref }
}

function suppressionKey(from: string, to: string, type: RepoEdgeType): string {
  return from + '->' + to + '::' + type
}
...
function optionalString(holder: Record<string, unknown>, field: string, what: string): string | undefined {
  const value = holder[field]
...
function validateNode(key: string, value: unknown): void {
  ...
  if (meta !== undefined) {
    if (!isObject(meta)) throw new TypeError(what + ': meta must be an object')
    for (const [metaKey, metaValue] of Object.entries(meta)) {
      const ok =
        typeof metaValue === 'string' ||
        (Array.isArray(metaValue) && metaValue.every(item => typeof item === 'string'))
      if (!ok) throw new TypeError(what + ': meta[' + JSON.stringify(metaKey) + '] must be a string or string array')
    }
  }
}

function validateEdge(value: unknown, nodeKeys: ReadonlySet<string>): void {
  ...
  if (!EDGE_TYPES.includes(type)) throw new TypeError(what + ': unknown type ' + JSON.stringify(type))
  if (!nodeKeys.has(from)) throw new TypeError(what + ': unknown from repo ' + JSON.stringify(from))
  if (!nodeKeys.has(to)) throw new TypeError(what + ': unknown to repo ' + JSON.stringify(to))
  ...
  if (ref !== undefined) {
    if (!isObject(ref)) throw new TypeError(what + ': contractRef must be an object')
  ...
  const expected = edgeId(from, to, type as RepoEdgeType, contractName)
  if (id !== expected) {
    throw new TypeError(what + ': id ' + JSON.stringify(id) + ' does not match identity ' + JSON.stringify(expected))
  }
}

function validateSuppressed(value: unknown): void {
...
export function validateRepoGraphDocument(value: unknown): asserts value is RepoGraphDocument {
  if (!isObject(value)) throw new TypeError('RepoGraphDocument: expected an object')
  if (value['version'] !== 1) {
  ...
  if (!isObject(nodes)) throw new TypeError('RepoGraphDocument.nodes: expected an object')
  for (const [key, node] of Object.entries(nodes)) validateNode(key, node)
  const nodeKeys = new Set(Object.keys(nodes))
  const edges = value['edges']
  if (!Array.isArray(edges)) throw new TypeError('RepoGraphDocument.edges: expected an array')
  for (const edge of edges) validateEdge(edge, nodeKeys)
  const suppressed = value['suppressed']
  if (!Array.isArray(suppressed)) throw new TypeError('RepoGraphDocument.suppressed: expected an array')
  for (const mark of suppressed) validateSuppressed(mark)
}
...
export class RepoGraphStore {
  ...
  private readonly edges = new Map<string, RepoEdge>()
  private readonly suppressed = new Map<string, SuppressedEdge>()

  private constructor(readonly project: string) {}

  /** Create an empty graph for one project (one related repo group, 12-1). */
  static create(project: string): RepoGraphStore {
    if (project === '') throw new TypeError('project must be a non-empty string')
  ...
  static load(doc: RepoGraphDocument): RepoGraphStore {
    ...
    for (const [key, node] of Object.entries(doc.nodes)) store.nodes.set(key, node)
    for (const edge of doc.edges) store.edges.set(edge.id, edge)
  ...
  toDocument(now: string = new Date().toISOString()): RepoGraphDocument {
    const nodes: Record<string, RepoNode> = {}
    ...
    return {
      ...
      edges: [...this.edges.values()].map(edge =>
        stripUndefined({ ...edge, contractRef: detachContractRef(edge.contractRef) }),
      ),
  ...
  upsertNode(key: string, node: RepoNode): void {
    if (key === '') throw new TypeError('node key must be a non-empty string')
    this.nodes.set(
      key,
      stripUndefined({
        ...node,
  ...
  removeNode(key: string): void {
    ...
    for (const [id, edge] of this.edges) {
      if (edge.from === key || edge.to === key) this.edges.delete(id)
    }
  }
  ...
  suppressEdge(id: string, reason?: string): void {
    ...
    const next = stripUndefined({ ...edge, status: 'suppressed' as const })
  ...
  confirmEdge(id: string): void {
    const edge = this.edges.get(id)
    if (edge === undefined) return
    this.edges.set(id, stripUndefined({ ...edge, status: 'confirmed' as const }))
    this.suppressed.delete(suppressionKey(edge.from, edge.to, edge.type))
  }
  ...
  mergeAutoEdges(inputs: Iterable<AutoEdgeInput>): MergeReport {
    const added: RepoEdge[] = []
    const updated: RepoEdge[] = []
    let skippedManual = 0
  ...
  private requireNode(key: string): void {
    if (!this.nodes.has(key)) throw new TypeError('unknown repo: ' + key)
  }
}
