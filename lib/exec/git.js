    ...
    for (const raw of lines(stdout)) {
        if (raw.startsWith('## ')) {
            ...
            if (head.startsWith('No commits yet on '))
                head = head.slice('No commits yet on '.length);
...
export class GitClient {
    runner;
    identity;
    constructor(runner, identity = { name: 'dsh-repo-board', email: 'repo-board@dsh.invalid' }) {
        this.runner = runner;
    ...
    async status(cwd) {
        return parseStatus(await this.require(['status', '--porcelain=v1', '--branch', '--untracked-files=all'], cwd));
    }
    /** Files changed vs `base` (or vs HEAD for staged+worktree changes). */
    async listChangedFiles(cwd, base) {
        const result = await this.git(base === undefined ? ['diff', '--name-only', 'HEAD'] : ['diff', '--name-only', base], cwd);
        if (result.exitCode === 0)
