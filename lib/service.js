...
export class RepoBoardService extends Service {
    ...
    async dispatchRequirement(record, task, options = {}) {
        ...
        if (record.status === 'dispatched') {
            ...
            if (run === undefined)
                throw new Error('repo board: dispatched requirement carries no run - cannot re-dispatch');
            const pending = run.perRepo.filter(entry => entry.state === 'submit-pending').map(entry => entry.repo);
            if (pending.length > 0) {
                throw new Error('repo board: repos still awaiting submit decisions (' + pending.sort().join(', ') +
            ...
            if (plans === undefined)
                throw new Error('repo board: dispatched requirement carries no plans');
        ...
        if (previousRun !== undefined) {
            ...
            const retainedErrors = previousRun.errors.filter(error => [...retainedRepos].some(repo => error.startsWith(repo + ':')));
            finalRun = {
    ...
    async approveSubmit(record, repo, options = {}) {
        ...
        if (branch !== entry.submitRequest.branch) {
            throw new Error('repo board: ' + repo + ' is on branch ' + branch + ' but its submit request expects ' +
