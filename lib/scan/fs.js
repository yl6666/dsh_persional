...
import { isContractFile } from "../extract/contract.js";
import { MANIFEST_FILES, contentFingerprint } from "../extract/manifest.js";
 ...
 * sources.
 */
export async function scanRepoDirectory(dir) {
    const manifest = {};
    const contracts = {};
    const sources = {};
    const walk = async (current) => {
        ...
        for (const entry of entries) {
            ...
            if (!entry.isFile() || isBinary(entry.name))
                continue;
 ...
 */
export function scannedFingerprint(files) {
